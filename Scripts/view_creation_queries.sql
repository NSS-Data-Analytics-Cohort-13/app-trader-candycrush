...
SELECT  distinct(a.name), SUM(CAST(a.review_count AS INTEGER) + p.review_count) AS total_review, ROUND(AVG(a.rating + p.rating)/2,2) AS avg_rating, a.primary_genre  AS genres,
...
/*
SELECT  distinct(a.name), SUM(CAST(a.review_count AS INTEGER) + p.review_count) AS total_review, ROUND(AVG(a.rating + p.rating),2) AS avg_rating, a.primary_genre  AS genres,
	CASE WHEN a.price >= CAST(REPLACE(p.price,'$','') AS DECIMAL) THEN a.price
	ELSE CAST(REPLACE(p.price,'$','') AS DECIMAL) END AS high_price
FROM app_store_apps AS a
INNER JOIN play_store_apps AS p
...
SELECT distinct(name), CAST(review_count AS INTEGER) AS total_review, rating, primary_genre AS genres, price FROM app_store_apps
UNION 
